...
class CRC_Fix:
    ...
    def __init__(self, src, tar, fix_crc=True, auto=True, timestamp=True, overwrite=False):
        self.tar = tar
        self.src_elems, self.tar_elems = self.src_elems(src), self.get_elems(tar)
        self.fix_crc, self.auto, self.overwrite, self.timestamp = fix_crc, auto, overwrite, timestamp
        self.fixed = list()
    def src_elems(self, src):
        elems_dict = {}
        for s in src:
    ...
    def print(self):
        if self.fixed:
            ...
            print('{:{ml}}{:<12}{:<12}{:<23}'.format('File Name', 'CRC', 'FIX', 'Modified',ml=ml))
            for e in elems:
                e_e = self.src_elems[e.name]
                print('{:{ml}}{:<12}{:<12}{:<23}'.format(e.name, hex(ifb(e.crc))[2:], hex(ifb(e_e.crc))[2:] , f'{e_e.dos_date()} {e_e.dos_time()}', ml=ml))
        ...
        with open(self.apk_crc, 'wb') as g:
            for elem in self.tar_elems.values():
                try:
                    ...
                    if self.fix_crc and not self.tar_elems[elem.name].crc == crc and (self.auto or elem.name.startswith('classes') and elem.name.endswith('.dex')):
...
if __name__ == '__main__':
    ...
    CRC_Fix(args.source, args.target, args.c, args.d, args.t, args.f).fix() if args.c or args.d and args.t else parser.print_help()